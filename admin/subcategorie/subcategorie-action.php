...
if(isset($_POST['btn_action']))
{
    ...
    if($_POST['btn_action'] == 'AJOUTER')
    {
        ...
        $count = !is_null($subcategories) ? count($subcategories) : 0;
        if($count > 0)
        {
            echo json_encode('subcategorie existante');
        }else
        {

            $photo = saveImage($_FILES["photo"], "subcategorie", "subcategorie")["url"];

            $res = insert("subcategories", [
                "label" => $_POST["label"],
    ...
    if($_POST['btn_action'] == 'changer_statut')
    {
        ...
        $res1 = update("subcategories", [
            "status" => $status,

        ], [
            ["id", $_POST["id_subcategorie"]]
        ]);
...
if(isset($_POST['btn_action_view'])) {
    ...
    if ($_POST['btn_action_view'] == 'consulter') {
        ...
        $subcategorie = verify_subcategory($_POST["id_subcategorie_view"]);

        $result = !is_null($subcategorie) ? array($subcategorie) : array();
        ...
        foreach ($result as $row) {
			...
			<tr>
                <td> Categorie </td>
                <td>' . verify_category($row["categorie_id"])["label"] . '</td>
			</tr>
			<tr>
                <td> Statut </td>
				<td>' . $status . '</td>
...
if(isset($_POST['btn_action_modif']))
{
    ...
    if($_POST['btn_action_modif'] == 'fetch_single')
    {
        ...
        $subcategorie = verify_subcategory($_POST["id_subcategorie_modif"]);
        
        if(!is_null($subcategorie)) {
            $subcategorie["categorie"] = verify_category($subcategorie["categorie_id"])["label"];
        }

        $output = !is_null($subcategorie) ? $subcategorie : array();
    ...
    if($_POST['btn_action_modif'] == 'Modifier')
    {
        ...
        $statement0->execute(
            array(
                ':id'	    =>	$_POST["id_subcategorie_modif"],
        ...
        }else
        {
            ...
            $res2 = update("subcategories", [
                'label' =>$_POST["label_modif"],
                'categorie_id' =>$_POST["categorie_modif"],
